...
Last updated: 2026-04-18 (Bevel adoption PR 5 staged — 7d EMA trend-line overlay on WeightTrendCard + Progress Body sub-tabs · preflight green · pending commit + push to `rial-food/main`)

## Release snapshot
- Root branch: `main`, HEAD `4622f76` (post-push snapshot PR 4). PR 5 staged on top of it: **7d EMA weight-trend overlay** on `WeightTrendCard` with half-life-7 smoothing (`EMA_ALPHA_7D ≈ 0.0943`, MacroFactor/Yazio semantics) + **Progress Body sub-tabs** (Summary/History/Calendar via `SegmentedTabs`). `weight-trend.ts` extends `WeightTrend` with `emaSeries`/`currentEma`/`emaWeekDelta`, exports `calcEmaSeries(values, alpha?)` (anchored at first value, geometric decay). `WeightTrendCard` rewritten with a simpler API (`{snapshots, targetKg, unitSystem, onLog, t}` replaces 11-prop legacy) — calls `calcWeightTrend` internally, renders 3-layer SVG (faded raw path opacity 0.22 + raw dots opacity 0.5 + solid EMA stroke 2.5) with stats grid **Tendencia 7d** / **Hoy** / **Cambio**, delegates log button to `onLog()` (no inline edit). Progress.tsx Body tab adds `SegmentedTabs` (summary default) mounting `BodyTimeline` + `BodyCalendar` on demand; weight-log flow delegated to global `useLogSnapshot()` pub-sub. Convention test `weight-ema.test.ts` (8 assertions) locks `EMA_ALPHA_7D`, anchor-to-first, monotonicity preservation, variance reduction, half-life midpoint (step 70→72 reaches ~71 after 7 samples), `(1−α)^7 ≈ 0.5`. i18n +7 keys × 2 locales (`trend7d`, `weightRawLabel`, `trendHint`, `bodySummary`, `bodyHistory`, `bodyCalendar`, `shared`). **Scope pivot vs plan:** originally PR 5 was Home ring-grid consolidation (IMG_0974) behind `featureFlags.homeRingGrid` — pivoted to weight-EMA because the owner had pre-existing uncommitted EMA work in the tree that revealed a clear MacroFactor/Yazio-style design intent. Home ring-grid deferred to PR 6 (requires `src/lib/featureFlags.ts` infrastructure first).
...
## Quality baseline (2026-04-18, PR 5 Bevel — 7d EMA weight-trend overlay + Progress Body sub-tabs)
- TypeScript: 0 errors (`npx tsc --noEmit`)
- Tests: **589/589** unit tests passing (+9 vs PR 4 baseline — 8 from `weight-ema.test.ts` convention + 1 from expanded Progress coverage; 580 → 589)
- i18n symmetry: **1530** keys aligned ES ↔ EN (`npm run check:i18n`) — +7 keys × 2 locales in PR 5 (`trend7d`, `weightRawLabel`, `trendHint`, `bodySummary`, `bodyHistory`, `bodyCalendar`, `shared`)
- Design-system lint: 0 errors, warnings pre-existing (type-debt `no-explicit-any` + 5 intentional shadcn allowlist entries)
- Build (measured 2026-04-18 post-PR-5-staged via `npm run release:preflight`):
  - main entry: **774.4 KB raw / 242.4 KB gzip** (+1.2 KB raw / +0.4 KB gzip vs PR 4 — EMA util + sub-tab state machine net-negligible; inline weight-edit removal offsets new tests)
  - vendor-recharts: ~331 KB raw / ~100 KB gzip (unchanged)
  - total dist/assets/*.js: ~2742 KB raw / ~781 KB gzip
