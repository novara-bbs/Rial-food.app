...
Last updated: 2026-04-18 (Bevel adoption PR 6b staged — `<BeforeAfterCompare>` photo comparator inside `<BodyTimeline>` closes MacroFactor/Yazio/Cronometer market gap; predecessor PR 6 `<BottomSheet>` V2 shipped at `6eed7fa`)

## Release snapshot
- Root branch: `main`, HEAD `6eed7fa` (PR 6 `<BottomSheet>` V2, in sync with `rial-food/main`). PR 6b `<BeforeAfterCompare>` staged on working tree (pending commit + push).
...
## Recent commits on `main` (in sync with `rial-food/main` at `6eed7fa`)
...
## Quality baseline (2026-04-18, PR 6b Bevel staged — `<BeforeAfterCompare>`; measured via `release:preflight`)
- TypeScript: 0 errors (`npx tsc --noEmit`)
- Tests: **619/619** unit tests passing (+6 vs PR 6 baseline — all from new `BeforeAfterCompare.test.ts`; 613 → 619)
- i18n symmetry: **1549** keys aligned ES ↔ EN (`npm run check:i18n`) — +11 keys × 2 locales in PR 6b (`compareCta`, `compareTitle`, `compareExit`, `compareNotEnough`, `compareSelectBefore`, `compareSelectAfter`, `compareBeforeLabel`, `compareAfterLabel`, `compareDaysPattern`, `compareSwap`, `compareReset`); 1538 → 1549
...
- Build (measured 2026-04-18 via `npm run release:preflight`):
  - main entry: **777.2 KB raw / 243.2 KB gzip** (+0.8 KB raw / +0.3 KB gzip vs PR 6 — BeforeAfterCompare component + daysBetweenISO util + wiring)
  - vendor-recharts: ~331.5 KB raw / ~99.8 KB gzip (unchanged)
  - total dist/assets/*.js: ~2755.2 KB raw / ~784.1 KB gzip
...
- **PR 6b — staged (pending commit + push)**. `<BeforeAfterCompare>` photo comparator — see Release snapshot above. CHANGELOG `[1.5.36]`.
...
**Governance.** Owner authorized push of PR 2, PR 3, PR 4, PR 5, PR 6 via "continua" directive after green preflight. Same pattern applies to PR 6b and PR 7.
