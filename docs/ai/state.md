...
Last updated: 2026-04-19 PM (PR 8 Bevel Home ring-grid `[1.5.43]` staged on working tree — `<NutritionHeroRing>` semi-ring 270° + 3-col macros behind `featureFlags.homeRingGrid` default-false; S1 + S2 cerrados al push)

## Release snapshot
- Root branch: `main`, HEAD in sync with `rial-food/main` (S1.2 `fe4863c` + S1.3 `83584ae` + S2.0 `aebba2e` + S2.1 `608d78f` + `3f112ce` pushed in session 2026-04-19 per governance memory `feedback_continua_push_approval.md`). PR 8 `[1.5.43]` staged on working tree (featureFlags + NutritionHeroRing + NutritionHero flag routing + Home ProgressPreviewCard guard + ringAriaLabel i18n + home-hero convention test + CHANGELOG).
- **PR 8 Bevel Home ring-grid staged (2026-04-19 PM).** S4 of the re-planned Bevel roadmap per `docs/market/home-patterns-benchmark.md` §6.1 — Option A hybrid. Gate-levantado: §6.2 GREEN (convergence ≥3/5 ring-family, macros layout decidido, ≥3 anti-patterns catalogados). Conservative roll-out: nuevo shape detrás de `featureFlags.homeRingGrid` default `false` → cero regresión visual para usuarios existentes; el opt-in es via env `VITE_FEATURE_HOME_RING_GRID=1` en un `.env.local` per-developer o preview env. Write set: (1) `src/lib/featureFlags.ts` **nuevo** — `Object.freeze({ homeRingGrid: readEnvFlag('VITE_FEATURE_HOME_RING_GRID') })` con helper SSR/vitest-tolerante que acepta `true|'true'|'1'`. (2) `src/features/home/components/NutritionHeroRing.tsx` **nuevo** (~228 líneas) — semi-ring 270° open-at-bottom via SVG handwritten (**zero recharts import, zero bundle delta** confirmado por `size:check`: main **777.8 KB raw / 243.4 KB gzip** unchanged). Exporta 2 pure helpers `describeSemiRingArc(cx, cy, r, progress)` + `describeSemiRingTrack(cx, cy, r)` con coordinate convention clockwise-from-top (`x = cx + r·sin(θ)`, `y = cy − r·cos(θ)`), `large-arc-flag = 1` cuando sweep > 180°, sweep-flag = 1 (clockwise). Anatomy: viewBox 180×180, cx=90 cy=90 r=72 strokeWidth=12, track `var(--surface-container-highest)` + progress `currentColor` via `text-primary`. Number hero centrado (`text-display text-primary tabular-nums`) + label `kcal remaining`. Running-sum caption dl `Objetivo − Alimentos + Ejercicio` preservando el patrón educativo MFP §3.4. 3-col macros row (carbs/protein/fats) con dot + bar + absolute (`consumed/target g`), colores via theme tokens (`bg-tertiary` / `bg-brand-secondary` / `bg-error`), **zero hex codes, zero `dark:` prefix**. a11y: contenedor `role="img" + aria-label={ringAria}` con `t.home.ringAriaLabel.replace('{remaining}', String(remaining))`. API idéntica a `NutritionHero` — caller no branchea. (3) `src/features/home/components/NutritionHero.tsx` — añadido import de `featureFlags` + `NutritionHeroRing` + branch `if (featureFlags.homeRingGrid) return <NutritionHeroRing …/>` después de `useI18n()` (rules-of-hooks respect). Legacy body literalmente untouched. (4) `src/features/home/screens/Home.tsx` — `ProgressPreviewCard` wrappeado en `{!featureFlags.homeRingGrid && (…)}` (evita duplicación semántica con el ring hero; flag-off preserva current Home exactamente). (5) `src/i18n/locales/es.ts` + `en.ts` — nueva key `t.home.ringAriaLabel` con placeholder `{remaining}`. i18n 1561 → **1562**. (6) `src/test/conventions/home-hero.test.ts` **nuevo** — 28 asserts en 6 describe blocks (static file-read pattern): featureFlags shape (4), `describeSemiRingArc` geometry (6), `describeSemiRingTrack` backdrop (2), NutritionHero flag routing + rules-of-hooks (5), NutritionHeroRing anatomy + no-recharts + token-purity (6), Home.tsx flag-gated ProgressPreviewCard (3), i18n symmetry (2). Preflight GREEN: tsc 0, lint 0 (pre-existing warnings only), tests **651 → 679** (+28 nuevos), i18n **1562** simétrico, build + size:check PASS. Preview verification flag-off default: legacy equation-hero visible (RESTANTE 1850 / OBJETIVO / − ALIMENTOS / + EJERCICIO), `ProgressPreviewCard "TU PROGRESO"` visible arriba del hero → cero regresión visual para el shape que ships hoy. Flag-on contract locked por las 28 convention test assertions (static file-read verifica routing + anatomy + token-purity sin necesidad de DOM render). **Rollback path**: flag a false o remove env var → 0 cambios de DB, 0 migraciones.
