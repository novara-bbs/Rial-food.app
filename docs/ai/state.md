...
Last updated: 2026-04-18 (Bevel adoption PR 4 staged — 3 sheet migrations to `<BottomSheet>` + ShareSheet dead-code purge · PR 3 still at `rial-food/main` HEAD `18938a6` · PR 4 verified via preview on CreateModal/LogSnapshotModal/RecipePicker, 0 console errors)

## Release snapshot
- Root branch: `main`, **PR 4 staged locally** on top of `18938a6` (which is in sync with `rial-food/main`). PR 4 migrates `LogSnapshotModal` (radix `Dialog` → `<BottomSheet>` + footer Cancelar/Guardar + sub-HIG `w-7 h-7` → `w-11 h-11` on photo remove + `text-sm` → tokens), `RecipePicker` (raw-div sheet → `<BottomSheet>` + API conditional-mount → controlled-open), `CreateModal` (radix `Dialog` → `<BottomSheet>`, preserves public `{isOpen, onClose, onSelect}` for App.tsx), and deletes `ShareSheet.tsx` (0 consumers — dead code since inception). `CreatePost.tsx` + `CreateStory.tsx` consumers updated for the new `RecipePicker` API. **Scope pivot vs plan:** the original 5 targets (`PhotoUploader`/`AddMeal`/`ImportRecipeURL`/`BarcodeScanner` + `LogSnapshotModal`) reduced to 1 real sheet (`LogSnapshotModal`) after audit — the other 4 were Dialog pickers / inline flows / full-screen overlays, not sheets. Discovered 3 new real sheets (`CreateModal`, `RecipePicker`, `ShareSheet`) during the audit. Home hero consolidation (IMG_0974) deferred to PR 5 — requires creating `src/lib/featureFlags.ts` infrastructure first. Verified via `preview_inspect` on live flows: all 3 sheets report `border-top-left-radius: 24px` + `max-height ≈ 88vh` + overlay `oklab(0 0 0 / 0.25)` + content `y=44` (status bar visible). Zero console errors across the 3 flows.
...
- Build (measured 2026-04-18 post-`18938a6` via `npm run release:preflight`):
  - main entry: **773.2 KB raw / 242.0 KB gzip** (+3.2 KB raw / +1.0 KB gzip vs PR 2 — theme matchMedia subscription + reorg markup)
  - vendor-recharts: 331.5 KB raw / 99.8 KB gzip (unchanged)
  - total dist/assets/*.js: ~2718 KB raw / ~774 KB gzip
