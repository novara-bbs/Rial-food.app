...
Last updated: **2026-05-01** — `[1.5.176]` MVP cleanup batch 1: seed image migration + FeatureErrorBoundary adoption.

## Release snapshot
- **Branch**: `main`, synced con `rial-food/main` (CI green S39; S40–S53 queued).
- **This session (2026-05-01, S53 — MVP cleanup [1.5.176])**:
    - **Refactor** `[1.5.176]` — MVP cleanup batch 1. Seed recipes (46 entries) migradas de `img:` a `image:` canonical, eliminando el workaround `?? recipe.img` en MealGapSuggestion. FeatureErrorBoundary wired en los 4 tabs principales (home/cocina/explore/more) en App.tsx; un crash en un tab ya no mata el resto de la app. `handleErrorReset` extraído como callback nombrado usado por feature y root boundaries. Test de MealGapSuggestion actualizado al nuevo contrato. Tests: 1464/1464 · size: 898.7 KB raw / 282.9 KB gzip.
- **Previous session (2026-05-01, S52 — home resilience [1.5.175])**:
...
## Quality baseline (post-refactor [1.5.175], 2026-05-01)
- TypeScript: **0 errors** (`npx tsc --noEmit`)
- Tests: **1464/1464** passing (103 files) — +28 from S52: ErrorBoundary (8) + meal-handlers (5) + safe-macros (8) + RecipeImage (4) + MealGapSuggestion (1) + img-onerror convention (2)
- i18n symmetry: **2007** keys aligned ES ↔ EN (1998 → 2007: nuevo namespace `errors.*`)
