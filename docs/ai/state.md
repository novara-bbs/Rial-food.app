...
Last updated: 2026-04-18 (Bevel adoption PR 6 staged — `<BottomSheet>` V2: focus size variant + 3 canonical header layouts + `hideHandle` + `leftSlot` escape hatch; no consumer migrations; predecessors `4ffc989` playbook re-audit + `cb674b9` WeeklyInsightsCard already shipped to `rial-food/main`)

## Release snapshot
- Root branch: `main`, HEAD `cb674b9` (WeeklyInsightsCard, in sync with `rial-food/main`). PR 6 `<BottomSheet>` V2 primitive extension staged on working tree (pending commit + push).
...
## Recent commits on `main` (in sync with `rial-food/main` at `cb674b9`)
...
## Quality baseline (2026-04-18, PR 6 Bevel staged — `<BottomSheet>` V2; measured with PR 6 staged + PR 6a `cb674b9` shipped on tree)
- TypeScript: 0 errors (`npx tsc --noEmit`)
- Tests: **613/613** unit tests passing (+24 vs PR 5 baseline — 15 from `bottom-sheet.test.ts` V2 expansion + 9 from new `week-insights.test.ts`; 589 → 613)
- i18n symmetry: **1538** keys aligned ES ↔ EN (`npm run check:i18n`) — +8 keys × 2 locales in PR 6a WeeklyInsightsCard (`weeklyInsights`, `weekInsightPositive`, `weekInsightNeutral`, `weekInsightLowData`, `weekInsightLowDataWithName`, `trendChip`, `streakChip`, `topMealChip`); 0 new i18n keys in PR 6 (primitive-only)
...
- Build (measured 2026-04-18 via `npm run release:preflight`):
  - main entry: **776.4 KB raw / 242.9 KB gzip** (+2.0 KB raw / +0.5 KB gzip vs PR 5 — WeeklyInsightsCard util + component net-small; PR 6 primitive extension adds ~0.3 KB gzip to ui vendor)
  - vendor-recharts: ~331.5 KB raw / ~99.8 KB gzip (unchanged)
  - total dist/assets/*.js: ~2746.8 KB raw / ~782.4 KB gzip
  - `size:check`: PASS (all budgets)
...
- **PR 6 — staged (pending commit + push)**. **Playbook re-audit** committed at `4ffc989` (docs-only, re-reads 64 Bevel captures and surfaces the focus-sheet typology + ConstantTile + Onboarding primitives; expands roadmap §5 from 4 → 9 PRs). **Primitive extension staged:** `<BottomSheet>` V2 — non-breaking addition of `size: 'compact' | 'focus'` (default `compact` = 88vh V1 unchanged; `focus` = 92vh for forms/searches/keyboard-first/detail-edit), `headerLayout: 'title-centered' | 'cancel-action' | 'back-title-action'` (default `title-centered` V1 unchanged; the two new ones render default "Cancelar" text button left for IMG_1004/0988 or default back chevron left for IMG_1015/1016), `hideHandle: boolean` (default `false`; hides swipe pill for keyboard-first IMG_1011 or navigation-stack IMG_1016), `leftSlot: ReactNode` escape hatch (IMG_1015 trash + title + add tri-column), plus `cancelLabel` / `backLabel` / `onBack` overrides. New exports: `BottomSheetSize` + `BottomSheetHeaderLayout` types. Emits `data-size` + `data-header-layout` attributes for introspection. Convention test `bottom-sheet.test.ts` expanded from 10 → **25 assertions** (V1 defaults fully locked + 15 new assertions for V2 types, defaults, `data-*` attrs, hideHandle gate, leftSlot nullish coalescing). `PRIMITIVES.md` sección `BottomSheet (ADR-009)` re-escrita con 3 ejemplos (compact / focus+cancel / focus+back+hideHandle) + size table + header-layout table. `ADR-009-bottom-sheet-anatomy.md` gains a full **V2 addendum** at the bottom. CHANGELOG `[1.5.35]`. **No consumer migrations in PR 6** — V1 consumers (`PortionSheet`, `PublishRecipeSheet`, `LogSnapshotModal`, `RecipePicker`, `CreateModal`) render identically unchanged. Consumer migrations deferred to PR 6.5+ selectivas por beneficio UX concreto (candidates in ADR-009 V2 addendum).
- **PR 7 (future)** — `<ConstantTile>` biometric tile primitive + Progress Body Summary grid (6 canonical states: loading, empty-no-template, empty-no-data, value-stable, value-trending-up, value-trending-down). See playbook §4.10.
- **PR 8 (future)** — Home hero ring-grid consolidation behind feature flag `featureFlags.homeRingGrid` (Bevel IMG_0974 pattern). Requires creating `src/lib/featureFlags.ts` first + NutritionHero/ProgressPreviewCard restructure.
- **PR 9 (future)** — Onboarding refactor: extract `<OnboardingScaffold>` + `<RadioCardGroup>` + `<SelectList>` primitives. See playbook §4.11.

**Governance.** Owner authorized push of PR 2, PR 3, PR 4, PR 5 via "continua" directive after green preflight. Same pattern applies to PR 6.
