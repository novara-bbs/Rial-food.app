...
## Recent commits on `main` (in sync with `rial-food/main` at `d9d7b77`)
...
## Quality baseline (2026-04-18, PR 7 Bevel staged — `<ConstantTile>` + Progress Body Summary grid; measured via `release:preflight`)
- TypeScript: 0 errors (`npx tsc --noEmit`)
- Tests: **645/645** unit tests passing (+26 vs PR 6b baseline — all from new `constant-tile.test.ts`; 619 → 645)
- i18n symmetry: **1560** keys aligned ES ↔ EN (`npm run check:i18n`) — +11 keys × 2 locales in PR 7 under `t.progress.constants.*` (`sectionTitle`, `weight`, `bmi`, `bodyFat`, `waist`, `hips`, `chest`, `noData`, `noRange`, `noTrends`, `stable`); 1549 → 1560
- Design-system lint: 0 errors, warnings pre-existing (type-debt `no-explicit-any` + 5 intentional shadcn allowlist entries). `eslint.config.mjs` primitive-shape-exception block expands from 2 → 3 files (adds `src/components/ConstantTile.tsx` alongside `SectionCard.tsx` + `surface.ts` — ADR-009 V2 §4.10 tile anatomy intentionally matches the Bevel tile-surface regex).
- Build (measured 2026-04-18 via `npm run release:preflight`):
  - main entry: **777.6 KB raw / 243.3 KB gzip** (+0.4 KB raw / +0.1 KB gzip vs PR 6b — ConstantTile primitive + body-constants aggregator + BodyConstantsGrid)
  - vendor-recharts: ~331.5 KB raw / ~99.8 KB gzip (unchanged)
  - total dist/assets/*.js: ~2763.2 KB raw / ~786.0 KB gzip
...
- **PR 7 — staged (pending commit + push)**. `<ConstantTile>` biometric tile primitive + Progress Body Summary grid — see Release snapshot above. CHANGELOG `[1.5.37]`.
...
**Governance.** Owner authorized push of PR 2, PR 3, PR 4, PR 5, PR 6, PR 6b via "continua" directive after green preflight. Same pattern applies to PR 7.
