...
Last updated: 2026-04-18 (Bevel adoption PR 6.5 staged — BottomSheet `focus` + `cancel-action` migrations: RecipePicker + LogSnapshotModal; predecessor PR 7 `<ConstantTile>` shipped at `0e11fe0`)

## Release snapshot
- Root branch: `main`, HEAD `0e11fe0` (PR 7 `<ConstantTile>`, in sync with `rial-food/main`). PR 6.5 staged on working tree (pending commit + push).
- **PR 6.5 staged.** Selective migrations of the existing `<BottomSheet>` consumers to ADR-009 V2 `size` + `headerLayout` variants. Audit rule: only consumers whose content matches the `focus`/"work on something" intent migrate; the rest stay `compact`. Migrations: (1) `RecipePicker.tsx` — `size="focus"` (keyboard-first search + list, IMG_1004 pattern); header stays `title-centered` because the flow is pick-and-close (no multi-step to justify `cancel-action`). (2) `LogSnapshotModal.tsx` — `size="focus" + headerLayout="cancel-action"`; Save button migrated from footer to `actionSlot` as text button, Cancel migrated to header via native radix Close; the `footer` prop is dropped entirely, gaining ~56px of content space and removing the close-X vs Cancel duplication. Stay-compact: `PortionSheet` (short picker), `PublishRecipeSheet` (short form, 1 textarea), `CreateModal` (FAB action grid, no input). Bug discovered + fixed in primitive: the `actionSlot` wrapper in `bottom-sheet.tsx` was `w-11 h-11` (fixed 44×44), clipping text buttons like "Guardar"/"Siguiente" that V2 `cancel-action` specifies — changed to `min-w-11 h-11` (preserves HIG minimum touch target while letting content auto-size). Convention test `bottom-sheet.test.ts` gains 1 new assertion locking `min-w-11 h-11 …-mr-2 …justify-end …shrink-0` against future regression. `Check` icon import removed from `LogSnapshotModal` (decorative icon no longer in the CTA — text-only "Guardar" matches Bevel IMG_1004/0988 exactly). No new i18n keys (cancelLabel fallback via `t.common.cancel` from V2; save text via `t.progress.save`). CHANGELOG `[1.5.38]`.
...
## Recent commits on `main` (in sync with `rial-food/main` at `0e11fe0`)
