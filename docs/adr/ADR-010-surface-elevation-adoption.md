...
- **2026-04-19 addendum — shadow sweep completed (CHANGELOG `[1.5.53]`).** Las ~11 ocurrencias de `shadow-{sm,md,lg,xl,2xl}` Tailwind default en `src/features/**` + `src/components/**` (fuera de las primitivas shadcn en `src/components/ui/**` y el demo-ribbon en `src/App.tsx`) migraron al scale `shadow-elev-{1,2,3}` según el mapping `sm→1 · md→2 · lg/xl/2xl→3`. Nueva regla ESLint `no-restricted-syntax` (`noTailwindShadow` + `noTailwindShadowTpl`) bloquea el uso de los 5 utilities Tailwind default fuera de la allowlist shadcn + App.tsx. Convention test `src/test/conventions/shadow-elevation.test.ts` complementa el ESLint con regresión en CI (BASELINE = 0). Adoption counter: 2 consumers (`SectionCard` + `BottomSheet`) → ≈13 consumers post-sweep. Colored shadow tints (`shadow-primary/25`, etc.) permanecen fuera de scope — son overlays decorativos, no parte del scale de elevación.
