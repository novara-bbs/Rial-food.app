...
| `font-body` | **Satoshi variable self-hosted** (wght 300..900, woff2 in `public/fonts/satoshi/`) | Primary reading, form fields, menu items |
...
`font-body` (Satoshi) is **self-hosted** as a variable woff2 under `public/fonts/satoshi/` — declared via `@font-face` in `src/index.css`. The previous Inter was Google-Fonts-loaded but ended up CSP-blocked in production by `font-src 'self'` (sprint `[1.5.108]`); self-hosting Satoshi solved the CSP and shipped a brand-stronger body face in one move. Bricolage Grotesque, JetBrains Mono and Fraunces remain on Google Fonts (loaded via `@import url(...)` in `src/index.css:1–2`) — they hit no CSP guard because `font-src` allows `https://fonts.gstatic.com`. Fraunces is conditional-fetched only when a `font-serif` class actually renders (editorial recipe hero). If you use `text-xs` inside a `<span className="font-label">` it renders JetBrains Mono — that is intentional. `font-serif` (Fraunces) is loaded but **not** applied by default — reach for it through `<Heading variant="editorial">` (ADR-012 primitive) or the raw `font-serif` Tailwind utility for verified-recipe titles (R2.5). One token, two consumers.
