...
export function getFoodInsights(realFeelLogs: any[], dictionary: Ingredient[]): FoodInsight[] {
  ...
  for (const log of linked) {
    for (const id of log.ingredientIds!) {
      if (!stats[id]) stats[id] = { levels: [], tags: [] };
      stats[id].levels.push(log.level);
      stats[id].tags.push(...(log.tags || []));
    }
  }

  const overallAvg = linked.reduce((s, l) => s + l.level, 0) / linked.length;
  const insights: FoodInsight[] = [];

  for (const [ingredientId, data] of Object.entries(stats)) {
    if (data.levels.length < 3) continue;

    const avgLevel = data.levels.reduce((s, v) => s + v, 0) / data.levels.length;
    const diff = avgLevel - overallAvg;

    const ingredient = dictionary.find(d => String(d.id) === ingredientId);
    const ingredientName = ingredient?.name || ingredientId;

    // Find most frequent tag
    const tagCounts: Record<string, number> = {};
    for (const t of data.tags) { tagCounts[t] = (tagCounts[t] || 0) + 1; }
    const dominantTag = Object.entries(tagCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || null;

    const tone: CorrelationTone = diff < -0.4 ? 'warning' : diff > 0.4 ? 'positive' : 'neutral';

    insights.push({ ingredientId, ingredientName, avgLevel, count: data.levels.length, tone, dominantTag });
  }

  return insights.sort((a, b) => Math.abs(b.avgLevel - overallAvg) - Math.abs(a.avgLevel - overallAvg)).slice(0, 10);
}
