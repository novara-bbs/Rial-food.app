      ...
      {insight.chips.length > 0 && (
        <div className="flex flex-wrap gap-2 pt-1">
          {insight.chips.map(chip => (
            <span
              key={chip.id}
              className={`inline-flex items-center gap-1 min-h-7 px-3 py-1 rounded-full bg-surface-container-high text-micro font-medium normal-case tracking-normal ${chipTone(chip)}`}
            >
              {chipIcon(chip)}
