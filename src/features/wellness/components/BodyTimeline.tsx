      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none">
        {FILTERS.map(f => (
          <button
            ...
            className={`shrink-0 px-3 py-1.5 rounded-full text-micro font-semibold uppercase tracking-widest transition-colors border ${
              filter === f.id
        ...
        {canCompare && (
          <button
            ...
            className={`shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-micro font-semibold uppercase tracking-widest transition-colors border ${
              compareMode
                ? 'bg-primary text-on-primary border-primary'
                : 'bg-surface-container-highest text-on-surface-variant border-outline-variant/20 hover:border-primary/50'
            }`}
