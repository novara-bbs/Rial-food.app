...
export type Goal = 'muscle' | 'cut' | 'maintain' | 'health' | 'family';
...
export function getFoodQuality(macros: {
  ...
  let score = 0;
  const fiberValue = fiber ?? macros.fiber;
  ...
  // Fiber bonus
  if (fiberValue && fiberValue > 5) score += 2;
  else if (fiberValue && fiberValue > 2) score += 1;
