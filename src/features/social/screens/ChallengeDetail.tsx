    ...
    <PageShell maxWidth="narrow" spacing="md">
      ...
      {!isJoined ? (
        <button
          ...
          className="w-full min-h-11 bg-primary text-on-primary rounded-sm font-headline text-body-sm font-bold uppercase tracking-widest hover:opacity-90 transition-opacity"
        >
          {ct.join}
        </button>
      ) : (
        <button
          type="button"
          onClick={() => handleCheckInChallenge(challengeId)}
          disabled={checkedInToday}
          className={`w-full min-h-11 rounded-sm font-headline text-body-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
            checkedInToday
              ? 'bg-surface-container-highest text-on-surface-variant cursor-default'
              : 'bg-primary text-on-primary hover:opacity-90'
          }`}
        >
          {checkedInToday ? <><Check className="w-4 h-4" /> {ct.checkedIn}</> : <><Calendar className="w-4 h-4" /> {ct.checkIn}</>}
        </button>
      )}
        ...
        <SectionCard padding="md" spacing="lg" title={ct.calendar}>
          <div className="grid grid-cols-7 gap-2">
            {calendarDays.map(d => (
              <div
                key={d.date}
                className={`aspect-square rounded-sm flex items-center justify-center text-caption font-bold transition-colors ${
      ...
      {isJoined && (
        <button
          ...
          className="w-full min-h-11 bg-surface-container-highest text-on-surface-variant rounded-sm font-headline text-body-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:text-error hover:border-error/50 border border-outline-variant/30 transition-colors"
        >
          <LogOut className="w-4 h-4" /> {ct.leave}
        </button>
      )}
    </PageShell>
  );
