    ...
    <PageShell maxWidth="wide" spacing="md">
      <div className="space-y-3">
        {MOCK_CREATORS.map(creator => {
          ...
          return (
            <div key={creator.id} className="bg-surface-container-low border border-outline-variant/20 rounded-sm p-5 hover:border-primary/50 transition-all">
              <div className="flex items-start gap-4">
                <button
                  type="button"
                  onClick={() => openProfile(creator.id)}
                  className="flex items-start gap-4 flex-1 min-w-0 text-left hover:opacity-90 transition-opacity"
                  aria-label={`${t.explore.creators.viewProfile} @${creator.name}`}
                >
                  <img src={creator.avatar} alt={creator.name} className="w-14 h-14 rounded-full object-cover border-2 border-outline-variant/20 shrink-0" referrerPolicy="no-referrer" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-headline font-bold text-sm uppercase text-tertiary">@{creator.name}</h3>
                      {creator.verified && <CheckCircle2 className="w-4 h-4 text-primary shrink-0" aria-label={t.explore.creators.verified} />}
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-3 text-xs text-on-surface-variant">
                      <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" aria-hidden="true" /> {(creator.followers / 1000).toFixed(1)}K {t.explore.creators.followers}</span>
                      <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" aria-hidden="true" /> {creator.recipes} {t.explore.creators.recipes}</span>
                    </div>
                  </div>
                </button>
                <button type="button"
