...
/**
 * Returns the shopping list with each item annotated with `inPantry: boolean`.
 * Matching is case-insensitive, partial-name aware (both directions).
 */
export function markPantryItems(
  shoppingItems: GroceryItem[],
  pantryItems: PantryItem[],
): (GroceryItem & { inPantry: boolean })[] {
  const pantryNames = pantryItems.map(p => p.name.toLowerCase().trim());

  return shoppingItems.map(item => {
    const itemName = item.name.toLowerCase().trim();
    const inPantry = pantryNames.some(
      pn => itemName.includes(pn) || pn.includes(itemName),
    );
