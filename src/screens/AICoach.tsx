...
import { ArrowLeft, Send, Bot, User, Sparkles, Lock, Crown, Zap, Activity, Utensils } from 'lucide-react';
...
export default function AICoach({ 
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      // Build the RAG Context (System Instruction)
      const systemInstruction = `
        ...
        Current Macros: ${JSON.stringify(memoryContext.dailyMacros)}
        Latest Check-in: ${JSON.stringify(memoryContext.checkInStatus)}
        Food Tolerance Logs: ${JSON.stringify(memoryContext.toleranceLogs)}
        Saved Recipes: ${(memoryContext.savedRecipes || []).slice(0, 5).map((r: any) => r.title).join(', ')}
      ...
      setMessages(prev => [...prev, { role: 'model', text: response.text || t.aiCoach.errorMessage }]);
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto h-full flex flex-col bg-surface">
      <header className="flex items-center gap-4 px-6 py-4 border-b border-outline-variant/20 shrink-0 bg-surface/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <div>
          <span className="font-label text-xs tracking-[0.2em] text-primary uppercase block">Función Pro</span>
          <h2 className="font-headline text-xl font-bold tracking-tighter uppercase text-tertiary flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-primary" /> Entrenador IA
          </h2>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto space-y-3">
          ...
          <div className="flex items-center gap-2">
            <input 
              ...
              className="flex-1 bg-surface-container-highest border border-outline-variant/30 py-3 px-4 text-sm font-label tracking-widest focus:outline-none focus:border-primary uppercase rounded-sm text-tertiary placeholder:text-on-surface-variant/50"
            />
            <button 
              onClick={() => handleSend()}
