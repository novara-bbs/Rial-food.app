...
export default function Pantry({ onBack }: { onBack: () => void }) {
  ...
  const addItem = (e: React.FormEvent) => {
    ...
    toast.success(`${newName} añadido a la despensa`);
    setNewName('');
    setNewQuantity('');
    setIsAdding(false);
  };
  ...
  const removeItem = (id: number) => {
    ...
    toast.success('Eliminado de la despensa');
  };
    ...
    <div className="px-6 max-w-4xl mx-auto space-y-8 pb-24">
      <header className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="w-10 h-10 bg-surface-container-low rounded-full flex items-center justify-center text-tertiary hover:bg-surface-container-highest transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <span className="font-label text-xs tracking-[0.2em] text-primary uppercase block">RIAL</span>
            <h2 className="font-headline text-2xl font-bold tracking-tighter uppercase text-tertiary">Mi Despensa</h2>
          </div>
        </div>
      ...
      <p className="text-sm text-on-surface-variant font-body leading-relaxed">
        Registra lo que tienes en casa. RIAL marcará estos ingredientes como disponibles cuando generes la lista de compras.
      </p>
      ...
      {isAdding && (
        <div className="bg-surface-container-high p-6 rounded-2xl border border-primary/30 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-headline text-sm font-bold uppercase tracking-widest text-primary">Añadir a despensa</h3>
          ...
          <form onSubmit={addItem} className="space-y-3">
            ...
            <input
              type="text"
              placeholder="Cantidad (ej. 500g, 1 bote)"
              value={newQuantity}
            ...
            <button
              ...
              className="w-full bg-primary text-on-primary py-3 rounded-lg font-headline font-bold text-xs uppercase tracking-widest hover:bg-primary-container transition-colors disabled:opacity-50"
            >
              Añadir
            </button>
          </form>
      ...
      <div className="space-y-8">
            ...
            <div className="space-y-2">
              {items.map(item => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex items-center gap-4 p-4 bg-surface-container-low border border-outline-variant/20 rounded-2xl group hover:border-primary/30 transition-colors"
                  >
        ...
        {pantryItems.length === 0 && !isAdding && (
          <EmptyState
            ...
            description="Añade ingredientes que ya tienes en casa."
