        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <span className="bg-primary/10 text-primary text-[9px] px-2 py-0.5 rounded font-bold uppercase tracking-widest">
              {(t.gamification.levels as any)[level.name] || level.name}
            </span>
      ...
      <div className="bg-surface-container-low border border-outline-variant/20 rounded-2xl p-5">
        ...
        {nextLevel && (
          <p className="text-[9px] text-on-surface-variant mt-2 uppercase tracking-widest">
            {(t.gamification.levels as any)[nextLevel.name]} — {nextLevel.minPoints - points} pts
          </p>
        )}
      </div>

      {/* Streak */}
      <div className="bg-surface-container-low border border-outline-variant/20 rounded-2xl p-5 flex items-center justify-between">
        ...
        <span className="font-mono text-3xl font-black text-orange-500">{stats.streakDays}</span>
      </div>
      ...
      <div className="space-y-3">
        {[
          ...
          <button key={idx} className="w-full flex items-center justify-between p-4 bg-surface-container-low border border-outline-variant/20 rounded-sm hover:border-primary/50 transition-all group text-left">
            ...
            <ChevronRight className="w-5 h-5 text-on-surface-variant/30 group-hover:text-primary" />
          </button>
        ))}
      </div>
    </div>
