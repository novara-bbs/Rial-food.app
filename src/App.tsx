...
export default function App() {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'recipe-detail': return <FeatureErrorBoundary featureName={t.recipes.title} onReset={handleErrorReset}><RecipeDetail recipe={selectedRecipe} onBack={() => navigateTo(previousScreen)} onSaveRecipe={handleSaveRecipe} isSaved={savedRecipes.some((r) => r.id === selectedRecipe?.id)} onAddToPlan={handleAddToPlan} onLogMealNow={handleLogMealNow} onAddToShoppingList={(items) => setShoppingList((prev) => [...prev, ...items])} dictionary={dictionary} userProfile={userProfile} /></FeatureErrorBoundary>;
      ...
      case 'profile': return <FeatureErrorBoundary featureName={t.profile.title} onReset={handleErrorReset}><Profile userProfile={userProfile} onBack={() => navigateTo(previousScreen)} realFeelLogs={realFeelLogs} savedRecipes={savedRecipes} communityPosts={communityPosts} nutritionHistory={nutritionHistory} dailyLogHasEntries={dailyLog.length > 0} /></FeatureErrorBoundary>;
      case 'settings': return <FeatureErrorBoundary featureName={t.settings.title} onReset={handleErrorReset}><Settings dailyMacros={dailyMacros} setDailyMacros={setDailyMacros} isPro={isProState} setIsPro={setIsPro} showAIBot={showAIBotState} setShowAIBot={setShowAIBot} userProfile={userProfile} setUserProfile={setUserProfile} dictionary={dictionary} hydration={hydration} setHydration={setHydration} movement={movement} setMovement={setMovement} onNavigateToLogin={() => setAuthScreen('login')} /></FeatureErrorBoundary>;
